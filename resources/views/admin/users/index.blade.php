            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('admin.users.index') }}" method="GET" class="mb-4">
                        <input type="text" name="search" placeholder="Search users..." class="border rounded px-2 py-1">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ $user->roles->pluck('name')->implode(', ') }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium">
                                        ...
                                        <form action="{{ route('admin.users.toggle-status', $user) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="ml-2 text-{{ $user->is_active ? 'red' : 'green' }}-600 hover:text-{{ $user->is_active ? 'red' : 'green' }}-900">
                ...
                <form action="{{ route('admin.users.change-role', $user) }}" method="POST">
                    ...
                    <select name="role">
                        ...
                        <option value="instructor" {{ $user->hasRole('instructor') ? 'selected' : '' }}>Instructor</option>
